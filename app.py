...
class Bird:
    ...
    def jump(self):
        self.vel = -10.5  # -ve velocity cauing goig upwards
        self.tick_count = 0
        self.height = self.y  # height of bird when it jumped
    ...
    while run:
        ...
        for x, bird in enumerate(birds):
            ...
            output = nets[x].activate((bird.y, abs(bird.y - pipes[pipe_ind].height), abs(
                bird.y - pipes[pipe_ind].bottom)))  # activate the neural network based on the inputs

            if output[0] > 0.5:  # this 0.5 is the threshold value generated from tanh activation function
                bird.jump()
        ...
        for pipe in pipes:
            for x, bird in enumerate(birds):
                if pipe.collide(bird):
                    ge[x].fitness -= 1  # penalize for hitting the pipe
                    birds.pop(x)
                    nets.pop(x)
                    ge.pop(x)
        ...
        if add_pipe:
            score += 1
            for g in ge:
                g.fitness += 5  # reward for passing the pipe
...
def run(config_path):
    ...
    p.add_reporter(neat.StdOutReporter(True))  # CONSOLE LOGS
...
if __name__ == "__main__":
    ...
    # weird fancy things that NEAT recommends
